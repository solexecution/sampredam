...
function hydrateServices() {
  ...
  if (filled(SITE.formspreeId)) {
    form.action = `https://formspree.io/f/${SITE.formspreeId}`;
  } else {
    // Show a notice that form isn't configured yet
    const wrapper = document.getElementById('contactFormWrapper');
    wrapper.classList.add('config-missing');
    wrapper.setAttribute('data-config-label', 'Formspree ID not set in config.js');
  }
}
...
function hydrateContact() {
  ...
  if (filled(CONTACT.phone)) {
    html.push(`
      <a href="tel:${CONTACT.phone}" class="contact-btn phone-btn" data-umami-event="contact-phone">
        ...
        <div><span>Call Us Directly</span><small>${CONTACT.phoneDisplay || CONTACT.phone}</small></div>
      </a>
    `);
  }
  ...
  if (filled(CONTACT.whatsapp)) {
    html.push(`
      <a href="https://wa.me/${CONTACT.whatsapp}?text=Hi%2C%20I'm%20interested%20in%20the%20property%20at%20${P.postcode || 'RG4 7QD'}" target="_blank" rel="noopener" class="contact-btn whatsapp-btn" data-umami-event="contact-whatsapp">
        ...
        <div><span>WhatsApp Us</span><small>Quick response guaranteed</small></div>
      </a>
    `);
  }
  ...
  if (filled(CONTACT.email)) {
    html.push(`
      <a href="mailto:${CONTACT.email}?subject=Property%20Enquiry%20-%20${P.postcode || 'RG4 7QD'}" class="contact-btn email-btn" data-umami-event="contact-email">
        ...
        <div><span>Send Email</span><small>${CONTACT.email}</small></div>
      </a>
    `);
  }

  if (html.length === 0) {
    buttons.innerHTML = '<div class="config-missing" data-config-label="Add contact details in config.js">Contact buttons will appear here once you add your phone, WhatsApp, or email to config.js</div>';
  } else {
